...
if __name__ == "__main__":
    ...
    ''' FINDME: REMOVE ALL ARGUMENTS THAT ARE INCLUDED IN THE DIRECTORY NAME: nnl, nnn, ne, nc, bs, lr, ts, rs 
                    AND DERIVE THEM FROM THE DIRECTORY NAME ITSELF
    '''
    ...
    # ap.add_argument('-nc', '--n_classes', type=int, required=False, default=-, help='n_classes == 1 for Regression (default); > 1 for Classification.')
    ...
    n_names = len(short_names)
    
    for k, sn, an in enumerate(zip(range(n_names, short_names, args_names))):
        args[an] = dir_parts[k+add_one].split(sn)[-1]
